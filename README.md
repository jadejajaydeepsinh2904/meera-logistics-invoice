...
V55 CLEAN SUPPLIER KHATA

Requested change:
- Truck-wise Pay button removed completely from Supplier Khata UI.
- Only one Pay Supplier button is shown for each supplier.
- Ledger View remains available.
- Linked truck numbers are shown as clean non-clicking chips only.
- Supplier row layout rebuilt to prevent mixed/overlapping buttons and nested-button HTML.
- Pending, Payable and Paid amounts are grouped cleanly on the right.
- Mobile layout stacks cleanly.
- All V54 dropdown/New Truck/New Supplier/Trip Supplier edit features remain unchanged.
- Database schema remains V53; no D1 migration is needed for V55.
- Invoice exact PDF View fix remains unchanged.

Deploy:
1. Upload complete V55 ZIP to GitHub main.
2. Vercel redeploy is enough for this UI-only change.
3. Open /?reset=v55 once.


V56 SUPPLIER-WISE TRUCK FILTER

Requested Pay Supplier behavior:
- Supplier selected hoy to Truck Number dropdown ma khali e supplier na j linked trucks aavshe.
- Global Pay Supplier open karo to pehla Supplier select karvo padse; badha trucks ek sathe nahi aave.
- Supplier badlo etle Truck dropdown turant e supplier pramane refresh thase.
- Supplier Khata na individual Pay Supplier button thi supplier preselected hoy, etle ena trucks direct aavshe.
- Existing/Edit payment ma current supplier/truck safely preserve thase.
- "＋ New Truck Add" option retained.
- Pay Supplier mathi New Truck Add karo to current selected supplier Owner Name ma preselected thase.
- Wrong supplier/truck combination submit thata pehla prevent thase.

No backend/D1 change.
Schema remains V53.
V55 clean Supplier Khata retained.
All V54 dropdown/New Supplier/Edit Supplier/Trip Supplier features retained.
Invoice View exact PDF fix retained.

Deploy:
1. Upload full V56 ZIP to GitHub main.
2. Vercel redeploy is enough for this frontend change.
3. Open /?reset=v56 once.


V57 KHATA UX CLEANUP

- Trip History (Transport Khata) renamed to Trip History everywhere in the main UI.
- Party Khata top/global Receive Payment button removed.
- Every Party card now has its own ₹ Receive Payment button.
- Party-wise Receive Payment opens with that Party selected and outstanding amount prefilled.
- Supplier Khata top/global Pay Supplier button removed.
- Every Supplier still has its own ₹ Pay Supplier and Ledger View buttons.
- Supplier ledger details are now visible directly on Supplier Khata like Party Khata.
- Supplier preview columns: Date, Type, Trip/Reference, Truck, Route/Detail, Payable, Paid, Balance.
- Freight entries, Supplier Payments and PM Bills are combined in the supplier preview.
- V56 supplier-wise truck filtering retained.
- No D1 schema change; schema_version remains 53.
- Exact Invoice View = Download PDF retained.

Deploy: upload full V57, redeploy Vercel, then open /?reset=v57 once.


V58 ACCOUNTING + DATA ISOLATION FINAL AUDIT

MAIN FIXES

1. Party payment allocation is now deterministic
- Party payment can be linked directly to Invoice.
- Trip-linked receipt maps to that Trip's Invoice.
- Old/unlinked receipts are allocated FIFO only once.
- One receipt can no longer be counted against every later invoice.
- Invoice received/pending values in Party Khata use backend allocation.
- Unallocated excess is kept as Party Advance/Credit.

2. Receive Payment form
- Party -> Invoice dropdown.
- Only that Party's invoices are shown.
- Invoice pending amount is visible.
- Selecting invoice prefills current pending amount.
- Blank Invoice means Party Advance / Auto FIFO.
- Invoice/Party and Trip/Party mismatches are rejected by backend.

3. Supplier accounting consistency
- Dashboard/Reports Supplier Payable now matches Supplier Khata calculation:
  Truck Entries + PM Bill supplier payable.
- Supplier paid/pending remain company scoped.

4. Multi-company hardening
- Critical runtime updates/deletes are scoped with company_id.
- Accounting audit checks cross-company parent/child links.
- Party/Trip/Invoice payment links are validated inside the logged-in company.

5. Reports & Audit
- New "Run Full Accounting Audit" button.
- Checks invoice/payment links, orphan rows, missing Truck/Supplier master,
  supplier/truck mismatch, cross-company links, and accounting totals.
- Shows Billing, Party Received/Outstanding, Supplier Payable/Paid/Pending,
  unallocated credit, error/warning counts.

6. Data safety
- Existing payments are not deleted or rewritten.
- New party_payments.invoice_id is added lazily once.
- Old payments continue to work through FIFO compatibility allocation.
- D1 schema_version remains 53; V58 adds only a small lazy accounting column/index.

RETAINED
- V57 Trip History naming + party-wise Receive Payment + inline Supplier ledger.
- V56 supplier-wise Truck filtering.
- V55 clean Supplier Khata.
- Full Truck/Supplier dropdown/edit features.
- Exact Invoice View = Download PDF.
- Multi-company SaaS/subscription/team foundation.

DEPLOY
1. Upload complete V58 ZIP to GitHub main.
2. Cloudflare Worker redeploy FIRST (backend accounting changes).
3. Vercel redeploy.
